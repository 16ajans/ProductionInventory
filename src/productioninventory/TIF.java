		...
		try (ImageInputStream in = ImageIO.createImageInputStream(path.toFile())) {
			...
			if (readers.hasNext()) {
				...
				try {
					...
					Node dpcWidth = (IIOMetadataNode) root.getElementsByTagName("HorizontalPixelSize").item(0)
							.getAttributes().item(0);
					Node dpcHeight = (IIOMetadataNode) root.getElementsByTagName("VerticalPixelSize").item(0)
							.getAttributes().item(0);
